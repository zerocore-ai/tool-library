//! Terminal CLI utility for listing and attaching to sessions.
//!
//! Usage:
//!   terminal-attach list              List active sessions
//!   terminal-attach attach <ID>       Attach to a session
//!   terminal-attach info <ID>         Show session details
...
#[derive(Parser)]
#[command(name = "terminal-attach")]
#[command(about = "Attach to terminal sessions managed by the terminal MCP server")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn main() {
    ...
    let result = match cli.command {
        ...
        Commands::Info { session_id } => cmd_info(&session_id),
    };
